...
class ConfirmHelper {
  ...
  static getItems(List lines) {
    ...
    for (int i = 0; i < lines.length; i++) {
      int quantity = 0;
      if (moneyExp.hasMatch(lines[i]) &&
          num.parse(moneyExp.stringMatch(lines[i]).toString()) < scanTotal) {
        ...
        if (scannedQuantity != null) {
          ...
          String parseQuantity = subQuantityExp.stringMatch(
              scannedQuantity); //in case quanitity has bracket ex. (3)
        ...
        if (calcTotal + lineCost <= scanTotal) {
          ...
          items
              .add(new Item(lines[i], lineCost, lineCost / quantity, quantity));
        }
      }
    }
  ...
  static getBoundingPolygon(List<WordBox> mergedLines) {
    for (int i = 0; i < mergedLines.length; i++) {
      ...
      ]); //top left corner, then clockwise
    ...
    for (int i = 0; i < mergedLines.length; i++) {
      ...
      for (int k = i; k < mergedLines.length; k++) {
        if (k != i && mergedLines[k].matched == false) {
          ...
          // print('OUTSIDE');
          if (insideCount == 4) {
            //all vertices are inside the bounding box
            // print('inside');
            //match array
